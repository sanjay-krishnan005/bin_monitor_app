...
Run seed data (after creating auth users):
...
|-------|-------------------------------------|
| superadmin@sortyx.demo | super_admin |
| admin@sortyx.demo | admin |
| customer@sortyx.demo | customer |
| tech@sortyx.demo | technician |
...
UPDATE profiles SET
  ...
  role = 'super_admin'
WHERE email = 'superadmin@sortyx.demo';
