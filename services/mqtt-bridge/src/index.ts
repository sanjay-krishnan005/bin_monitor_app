...
const telemetryPayloadSchema = z.object({
  ...
  cameraStatus: z.enum(["ok", "error"]).optional().default("ok"),
  sensorHealth: z.enum(["ok", "error"]).optional().default("ok"),
  internetStatus: z.enum(["online", "offline"]).optional().default("online"),
...
async function processTelemetry(topic: string, payload: Buffer) {
  ...
  await supabase.from("telemetry_events").insert({
    ...
    camera_status: data.cameraStatus,
  ...
  await supabase
    ...
    .update({
      ...
      sensor_health: data.sensorHealth,
      internet_status: data.internetStatus,
      status: data.internetStatus === "offline" ? "offline" : "active",
