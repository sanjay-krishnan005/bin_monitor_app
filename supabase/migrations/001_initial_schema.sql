...
-- Enums
CREATE TYPE user_role AS ENUM ('super_admin', 'admin', 'customer', 'technician');
...
CREATE POLICY org_select ON organizations FOR SELECT USING (
  public.is_super_admin() OR id = public.get_user_org_id()
);
CREATE POLICY org_all_super ON organizations FOR ALL USING (public.is_super_admin());

-- Customers policies
CREATE POLICY customers_select ON customers FOR SELECT USING (
  public.is_super_admin() OR organization_id = public.get_user_org_id()
);
CREATE POLICY customers_insert ON customers FOR INSERT WITH CHECK (
  public.is_super_admin() OR (public.get_user_role() IN ('admin', 'super_admin') AND organization_id = public.get_user_org_id())
);
...
CREATE POLICY profiles_select ON profiles FOR SELECT USING (
  id = auth.uid() OR public.is_super_admin() OR organization_id = public.get_user_org_id()
);
...
CREATE POLICY bins_select ON bins FOR SELECT USING (
  ...
  OR (public.get_user_role() IN ('admin', 'technician') AND organization_id = public.get_user_org_id())
...
CREATE POLICY bins_update ON bins FOR UPDATE USING (
  public.is_super_admin()
  OR (public.get_user_role() IN ('admin', 'super_admin') AND organization_id = public.get_user_org_id())
  OR (public.get_user_role() = 'technician' AND organization_id = public.get_user_org_id())
);
...
CREATE POLICY compartments_select ON bin_compartments FOR SELECT USING (
  EXISTS (SELECT 1 FROM bins b WHERE b.id = bin_id AND (
    public.is_super_admin()
    OR (public.get_user_role() IN ('admin', 'technician') AND b.organization_id = public.get_user_org_id())
...
CREATE POLICY compartments_update ON bin_compartments FOR UPDATE USING (
  EXISTS (SELECT 1 FROM bins b WHERE b.id = bin_id AND (
    public.is_super_admin()
    OR public.get_user_role() IN ('admin', 'super_admin', 'technician')
  ))
);
...
CREATE POLICY bin_ads_select ON bin_ads FOR SELECT USING (
  EXISTS (SELECT 1 FROM bins b WHERE b.id = bin_id AND (
    public.is_super_admin() OR b.organization_id = public.get_user_org_id()
    OR (public.get_user_role() = 'customer' AND b.customer_id = public.get_user_customer_id())
  ))
);
CREATE POLICY bin_ads_manage ON bin_ads FOR ALL USING (
  public.is_super_admin() OR public.get_user_role() IN ('admin', 'super_admin')
);
